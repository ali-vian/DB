...
--
-- Host: 127.0.0.1
-- Waktu pembuatan: 28 Bulan Mei 2024 pada 05.58
...
-- Prosedur
--
CREATE DEFINER=`` PROCEDURE `cekProduct` (IN `product_number` VARCHAR(10))   BEGIN SELECT * FROM products WHERE productCode=product_number; END$$

CREATE DEFINER=`` PROCEDURE `cobaIF` (IN `bil` INT(3))   BEGIN 
DECLARE str VARCHAR(50);
IF(bil < 0) THEN SET str = "Bilangan Negatif";
ELSE SET str = "Bilangan Positif";
END IF; SELECT str;
END$$

CREATE DEFINER=`` PROCEDURE `ExLoop` (IN `bil` INT(3))   BEGIN
...
CREATE DEFINER=`` PROCEDURE `getCountEmployees` (OUT `countEmployees` INT(10))   BEGIN SELECT COUNT(*) INTO countEmployees FROM employees; END$$

CREATE DEFINER=`` PROCEDURE `getCountEmployeesInout` (INOUT `jumlah` INT(10), IN `negara` VARCHAR(10))   BEGIN SELECT COUNT(*) INTO jumlah FROM employees 
JOIN offices USING(officeCode) WHERE country=negara; END$$

CREATE DEFINER=`` PROCEDURE `getEmployees` ()   BEGIN SELECT firstName,lastName,city,country FROM employees JOIN offices USING(officeCode); END$$

CREATE DEFINER=`` PROCEDURE `getEmployeesByCountry` (`namaNegara` VARCHAR(50))   BEGIN SELECT firstName,lastName,city,country FROM employees JOIN offices USING(officeCode) WHERE country=namaNegara; END$$

CREATE DEFINER=`` PROCEDURE `getOrder` (IN `order_number` VARCHAR(10))   BEGIN SELECT orderNumber, SUM(priceEach*quantityOrdered) AS total_order FROM orders JOIN orderdetails USING(orderNumber) WHERE orderNumber=order_number GROUP BY orderNumber; END$$

CREATE DEFINER=`` PROCEDURE `getOrdersLevel` (IN `order_Number` INT(10))   BEGIN
    SELECT orderNumber,SUM(quantityOrdered*priceEach) as total ,orderLevel(SUM(quantityOrdered*priceEach)) as ket FROM orderdetails WHERE orderNumber = order_Number;
END$$

CREATE DEFINER=`` PROCEDURE `getProductLevel` (IN `productId` VARCHAR(20), OUT `productLevel` VARCHAR(20))   BEGIN
	DECLARE price DEC(10,2) DEFAULT 0;
    SELECT buyPrice INTO price FROM products WHERE productCode = productId;
    SET productLevel = productLevel(price);
END$$

CREATE DEFINER=`` PROCEDURE `RetrieveData` ()   BEGIN
...
-- Fungsi
--
CREATE DEFINER=`` FUNCTION `orderLevel` (`price` DECIMAL(10,2)) RETURNS VARCHAR(20) CHARSET utf8mb4 DETERMINISTIC BEGIN 
DECLARE orderLevel VARCHAR(20);
	IF price > 1000 THEN
		SET orderLevel = "Jumbo";
    ELSEIF price >= 500 AND price <= 1000 THEN
		SET orderLevel = "Banyak";
    ELSEIF price < 50 THEN
		SET orderLevel = "Lumayan";
    END IF;
RETURN(orderLevel);
END$$

CREATE DEFINER=`` FUNCTION `productLevel` (`price` DECIMAL(10,2)) RETURNS VARCHAR(20) CHARSET utf8mb4 DETERMINISTIC BEGIN 
DECLARE productLevel VARCHAR(20);
	IF price > 100 THEN
		SET productLevel = "PLATINUM";
    ELSEIF price >= 50 AND price <= 100 THEN
		SET productLevel = "GOLD";
    ELSEIF price < 50 THEN
		SET productLevel = "SILVER";
    END IF;
RETURN(productLevel);
END$$
...
--
-- Stand-in struktur untuk tampilan `getcountordercustomer`
-- (Lihat di bawah untuk tampilan aktual)
--
CREATE TABLE `getcountordercustomer` (
...
--
-- Stand-in struktur untuk tampilan `getjumlahcustomer`
-- (Lihat di bawah untuk tampilan aktual)
--
CREATE TABLE `getjumlahcustomer` (
`nama_karyawan` varchar(101)
,`jumlah_customer` bigint(21)
);

-- --------------------------------------------------------

--
-- Stand-in struktur untuk tampilan `getorders`
-- (Lihat di bawah untuk tampilan aktual)
--
...
--
-- Stand-in struktur untuk tampilan `total_order`
-- (Lihat di bawah untuk tampilan aktual)
--
...
--
-- Struktur untuk view `getcountordercustomer`
--
DROP TABLE IF EXISTS `getcountordercustomer`;

CREATE ALGORITHM=UNDEFINED DEFINER=`` SQL SECURITY DEFINER VIEW `getcountordercustomer`  AS SELECT `customers`.`customerName` AS `customerName`, count(`customers`.`customerNumber`) AS `jumlah_order` FROM (`customers` join `orders` on(`customers`.`customerNumber` = `orders`.`customerNumber`)) GROUP BY `customers`.`customerNumber``customerNumber`  ;

-- --------------------------------------------------------

--
-- Struktur untuk view `getjumlahcustomer`
--
DROP TABLE IF EXISTS `getjumlahcustomer`;

CREATE ALGORITHM=UNDEFINED DEFINER=`` SQL SECURITY DEFINER VIEW `getjumlahcustomer`  AS SELECT concat(`employees`.`firstName`,' ',`employees`.`lastName`) AS `nama_karyawan`, count(`customers`.`salesRepEmployeeNumber`) AS `jumlah_customer` FROM (`employees` join `customers` on(`employees`.`employeeNumber` = `customers`.`salesRepEmployeeNumber`)) GROUP BY `customers`.`salesRepEmployeeNumber``salesRepEmployeeNumber`  ;

-- --------------------------------------------------------

--
-- Struktur untuk view `getorders`
--
DROP TABLE IF EXISTS `getorders`;

CREATE ALGORITHM=UNDEFINED DEFINER=`` SQL SECURITY DEFINER VIEW `getorders`  AS SELECT `orderdetails`.`orderNumber` AS `orderNumber`, `products`.`productName` AS `productName` FROM (`orderdetails` join `products` on(`orderdetails`.`productCode` = `products`.`productCode`))  ;

-- --------------------------------------------------------

--
-- Struktur untuk view `total_order`
--
DROP TABLE IF EXISTS `total_order`;

CREATE ALGORITHM=UNDEFINED DEFINER=`` SQL SECURITY DEFINER VIEW `total_order`  AS SELECT `orderdetails`.`orderNumber` AS `orderNUmber`, sum(`orderdetails`.`priceEach` * `orderdetails`.`quantityOrdered`) AS `total` FROM `orderdetails` GROUP BY `orderdetails`.`orderNumber` ORDER BY sum(`orderdetails`.`priceEach` * `orderdetails`.`quantityOrdered`) AS `DESCdesc` ASC  ;
